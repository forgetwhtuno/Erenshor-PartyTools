...
namespace ErenshorPartyTools
{
    internal static class SuiteUiPolicy
    {
        ...
        {
            _hubPresence = null; _hubState = new SuiteHubPresenceState(false, false, false); _nextHubProbe = 0f;
            if (owner == null) return;
            try { _hubPresence = owner.IPCAuraSubscriber<string>(HubPresenceEndpoint); } catch { _hubPresence = null; }
        }

        internal static bool IsGameplayReady()
        {
            if (!RawGameplayReady()) { _rawReadySince = -1f; _readySceneHandle = int.MinValue; _canMoveLatched = false; _acquired = false; return false; }
            Scene scene = SceneManager.GetActiveScene();
            if (_readySceneHandle != scene.handle) { _readySceneHandle = scene.handle; _rawReadySince = Time.unscaledTime; _canMoveLatched = false; _acquired = false; }
            ...
            _acquired = true; return true;
        }

        internal static bool IsHubPresent() { ProbeHub(); return _hubState.Present; }
        internal static bool IsHubAvailable() { ProbeHub(); return _hubState.Usable; }
        internal static bool IsHubQuickCloseVerified() { ProbeHub(); return _hubState.QuickCloseVerified; }

        private static void ProbeHub()
        {
            if (Time.unscaledTime < _nextHubProbe) return;
            _nextHubProbe = Time.unscaledTime + HubProbeSeconds; _hubState = new SuiteHubPresenceState(false, false, false);
            ...
            catch { _hubState = endpointPresent ? new SuiteHubPresenceState(true, false, false) : new SuiteHubPresenceState(false, false, false); }
        }

        internal static void Reset()
        {
            _rawReadySince = -1f; _readySceneHandle = int.MinValue; _canMoveLatched = false; _acquired = false;
            _nextHubProbe = 0f; _hubState = new SuiteHubPresenceState(false, false, false); _hubPresence = null;
        }
        ...
        private static bool RawGameplayReady()
        {
            try
            {
                ...
                return GameData.SimMngr != null && GameData.SimPlayerGrouping != null && GameData.GroupMembers != null;
            }
            catch { return false; }
        }
    }
