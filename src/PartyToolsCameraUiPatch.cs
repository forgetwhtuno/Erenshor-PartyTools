...
{
    internal static class PartyToolsCameraCompatibility
    {
    ...
    internal static class PartyToolsCameraUsingUiPatch
    {
        internal static bool ShapeVerified { get; private set; }
        [HarmonyPrepare] private static bool Prepare() { MethodInfo method; ShapeVerified = PartyToolsCameraCompatibility.Verify(out method); return ShapeVerified; }
        [HarmonyPostfix] private static void Postfix(ref bool __result) { try { if (!__result && PartyToolsDragGuard.OwnsPointerGesture) __result = true; } catch { } }
    }
}
